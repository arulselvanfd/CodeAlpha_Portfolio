...
const io = new IntersectionObserver((entries)=>{
  entries.forEach(e=>{
    if(e.isIntersecting){
      ...
      if(e.target.id === 'radarBox'){
        document.getElementById('radarShape').classList.add('animate');
      }
...
toggle.addEventListener('click', ()=>{
  ...
  document.documentElement.style.setProperty(
    '--bg',
    isDark ? '#111214' : '#ffffff'
  );

  document.documentElement.style.setProperty(
    '--bg-1',
    isDark ? '#17181a' : '#fafafa'
  );

  document.documentElement.style.setProperty(
    '--ink',
    isDark ? '#f4f4f5' : '#17181a'
  );

  document.documentElement.style.setProperty(
    '--ink-dim',
    isDark ? '#b4b7bb' : '#55585c'
  );

  document.documentElement.style.setProperty(
    '--ink-faint',
    isDark ? '#84878b' : '#8b8e92'
  );
